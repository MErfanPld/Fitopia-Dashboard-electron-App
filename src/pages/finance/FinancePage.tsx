...
export const FinancePage: React.FC = () => {
  ...
  const submitTx = async () => {
    ...
    try {
      ...
      showToast(e instanceof Error ? e.message : 'خطا در ثبت تراکنش', 'danger');
  ...
  const submitPay = async () => {
    ...
    try {
      await financeService.createPayment(gymId, {
        ...
        total_price: Number(payTotal),
        amount_paid: Number(payPaid ?? payTotal),
      ...
      showToast(e instanceof Error ? e.message : 'خطا در ثبت پرداخت', 'danger');
    } finally { setSaving(false); }
  };
  ...
  const submitRefund = async () => {
    ...
    try {
      ...
      showToast(e instanceof Error ? e.message : 'خطا در ثبت استرداد', 'danger');
    } finally { setSaving(false); }
  };
  ...
  const payColumns: Column<CustomerPayment>[] = [
    ...
    { key: 'total_price', header: 'مبلغ کل', render: (r) => <span className="text-sm tabular-nums">{money((r.total_amount ?? r.total_price))}</span> },
    { key: 'amount_paid', header: 'پرداخت‌شده', render: (r) => <span className="text-sm tabular-nums">{money((r.paid_amount ?? r.amount_paid))}</span> },
    ...
    <div className="space-y-4">
      ...
      {section === 'report' && report && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          <StatCard title="کل درآمد" value={money(report.total_income)} icon={TrendingUp} />
          <StatCard title="کل هزینه" value={money(report.total_expense)} icon={TrendingDown} />
          <StatCard title="مانده" value={money((report.total_income || 0) - (report.total_expense || 0))} icon={Wallet} />
          <StatCard title="تعداد تراکنش" value={String(report.transaction_count ?? txs.length)} icon={RefreshCw} />
        </div>
      )}
      ...
      <Modal isOpen={txOpen} onClose={() => setTxOpen(false)} title="ثبت تراکنش">
        <div className="space-y-3">
          ...
          <FormField label="شرح" value={txDesc} onChange={(e) => setTxDesc(e.target.value)} />
          <FormField label="شماره پیگیری" value={txRef} onChange={(e) => setTxRef(e.target.value)} />
          <SearchableSelect label="عضو (اختیاری)" value={txCustomer} onChange={setTxCustomer} options={memberOptions} placeholder="جستجوی عضو..." />
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" disabled={saving} onClick={() => setTxOpen(false)} className="px-4 py-2 text-sm rounded-lg text-muted hover:bg-surface-hover">انصراف</button>
      ...
      <Modal isOpen={payOpen} onClose={() => setPayOpen(false)} title="ثبت پرداخت">
        <div className="space-y-3">
          <SearchableSelect label="عضو" required value={payCustomer} onChange={setPayCustomer} options={memberOptions} placeholder="جستجوی عضو..." />
          ...
          <FormField label="شرح" value={payDesc} onChange={(e) => setPayDesc(e.target.value)} />
          <FormField label="شماره پیگیری" value={payRef} onChange={(e) => setPayRef(e.target.value)} />
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" disabled={saving} onClick={() => setPayOpen(false)} className="px-4 py-2 text-sm rounded-lg text-muted hover:bg-surface-hover">انصراف</button>
      ...
      <Modal isOpen={refundOpen} onClose={() => setRefundOpen(false)} title="ثبت استرداد">
        <div className="space-y-3">
          <SearchableSelect label="تراکنش درآمدی" required value={refundTx} onChange={setRefundTx} options={incomeTxOptions} placeholder="انتخاب تراکنش..." />
          <FormField label="مبلغ استرداد (تومان)" required type="number" min={0} value={refundAmount} onChange={(e) => setRefundAmount(e.target.value)} />
          <FormField label="دلیل" value={refundReason} onChange={(e) => setRefundReason(e.target.value)} />
          <div className="flex justify-end gap-2 pt-2">
            <button type="button" disabled={saving} onClick={() => setRefundOpen(false)} className="px-4 py-2 text-sm rounded-lg text-muted hover:bg-surface-hover">انصراف</button>
